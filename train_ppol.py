...
def train(args: TrainCfg):
    ...
    demo_env = gym.make('SafeMetaDrive-training')
    ...
    train_envs = worker([lambda: gym.make('SafeMetaDrive-training') for _ in range(training_num)])
    test_envs = worker([lambda: gym.make('SafeMetaDrive-validation') for _ in range(args.testing_num)])
